...
@section('contenido')
    <h1 class="w-full text-center text-gray-700 dark:text-gray-400 font-bold text-2xl mb-5">Editar Usuario</h1>
        ...
        <form action="{{ route('user.update', [$user->id])}}" method="post" class="w-full">
            ...
            <div class="relative z-0 w-11/12 mx-auto mb-5 group">
                @error('id_role')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
                <select name="id_role" id="id_role" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-auto dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer @error('id_role') border-red-500 @enderror">
                    @foreach ($roles as $role)
                        <option value="{{ $role->id }}" {{ $user->id_role == $role->id ? 'selected' : '' }}>{{ $role->role }}</option>
                    @endforeach
                </select>
                <label for="id_role" class="peer-focus:font-medium absoliute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placehorlder-shown:translate-y-0 peer-focus:scale-75 peer-focus:translate-y-6">Rol</label>
            </div>
            <button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full mx-auto block sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Guardar</button>
        </form>
    </div>
@endsection
